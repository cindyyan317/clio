...
{
    Subscription sub(ctx);
    std::shared_ptr<ServerNG::ConnectionBase> session1(new MockDeadSession(tagDecoratorFactory));
    sub.subscribe(session1);
    ctx.run();
    EXPECT_EQ(sub.count(), 1);
    // trigger dead
...
TEST_F(SubscriptionMapTest, SubscriptionMapCount)
{
    std::shared_ptr<ServerNG::ConnectionBase> session1 = std::make_shared<MockSession>(tagDecoratorFactory);
    std::shared_ptr<ServerNG::ConnectionBase> session2 = std::make_shared<MockSession>(tagDecoratorFactory);
    std::shared_ptr<ServerNG::ConnectionBase> session3 = std::make_shared<MockSession>(tagDecoratorFactory);
