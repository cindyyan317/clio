...
private:
    ...
    void
    canonicalize(SHAMapHash const& hash, std::shared_ptr<SHAMapTreeNode>&) const;
    ...
    SHAMapLeafNode*
    firstBelow(std::shared_ptr<SHAMapTreeNode>, SharedPtrNodeStack& stack, int branch = 0) const;

    // returns the last item at or below this node
    SHAMapLeafNode*
    lastBelow(std::shared_ptr<SHAMapTreeNode> node, SharedPtrNodeStack& stack, int branch = branchFactor) const;
