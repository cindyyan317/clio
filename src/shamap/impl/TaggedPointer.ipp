...
constexpr size_t elementSizeBytes = (sizeof(SHAMapHash) + sizeof(std::shared_ptr<SHAMapTreeNode>));
...
inline TaggedPointer::TaggedPointer(RawAllocateTag, std::uint8_t numChildren)
{
    ...
    assert((reinterpret_cast<std::uintptr_t>(p) & ptrMask) == reinterpret_cast<std::uintptr_t>(p));
        ...
        *this = std::move(other);
        ...
        for (int i = 0; i < SHAMapInnerNode::branchFactor; ++i) {
            ...
            if (inSrc && inDst) {
                ...
                new (&dstChildren[dstIndex]) std::shared_ptr<SHAMapTreeNode>{std::move(srcChildren[srcIndex])};
                ++dstIndex;
                ++srcIndex;
            } else if (inSrc && !inDst) {
                // remove
    ...
    : TaggedPointer(std::move(other))
{
    ...
    if (newNumAllocated == SHAMapInnerNode::branchFactor) {
        ...
        iterNonEmptyChildIndexes(isBranch, [&](auto branchNum, auto indexNum) {
            ...
            new (&newChildren[branchNum]) std::shared_ptr<SHAMapTreeNode>{std::move(oldChildren[indexNum])};
        });
        ...
        iterNonEmptyChildIndexes(isBranch, [&](auto branchNum, auto indexNum) {
            ...
            new (&newChildren[curCompressedIndex]) std::shared_ptr<SHAMapTreeNode>{std::move(oldChildren[indexNum])};
            ++curCompressedIndex;
        });
        // Run the constructors for the remaining elements
...
[[nodiscard]] inline std::tuple<std::uint8_t, SHAMapHash*, std::shared_ptr<SHAMapTreeNode>*>
TaggedPointer::getHashesAndChildren() const
{
    ...
    auto const children = reinterpret_cast<std::shared_ptr<SHAMapTreeNode>*>(hashes + numAllocated);
